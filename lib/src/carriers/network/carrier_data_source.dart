...
import '../dto/carrier_output_dto.dart';
import 'carrier_enums.dart';

@LazySingleton()
class CarrierDataSource {
  final Dio _dio;
  final BaseConfig _baseConfig;

  CarrierDataSource(this._dio, this._baseConfig);

  Future<RemoteResponse<CarrierOutputDTO>> getCarriers({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (response.data.toString() == '[]') {
          return const RemoteResponse<CarrierOutputDTO>(
            CarrierOutputDTO(carrierDTOList: []),
          );
        }

        final jsonMap = response.data as Map<String, dynamic>;

        final carrierOutputDTO = CarrierOutputDTO.fromJson(jsonMap);

        return RemoteResponse<CarrierOutputDTO>(carrierOutputDTO);
      } else {
  ...
  Future<RemoteResponse<CarrierOutputDTO>> getCarriersPage({
    required int languageId,
    required int page,
    ...
    final uriBuilder =
        UriBuilder(_baseConfig, carriersPath, languageId: languageId)
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        CarrierOutputDTO carrierOutputDTO = CarrierOutputDTO.fromJson(jsonMap);

        final carrierDTOList = carrierOutputDTO.carrierDTOList;
        bool? isNextPageAvailable = false;

        if (carrierDTOList.length > perPage) {
          carrierOutputDTO = carrierOutputDTO.copyWith(
            carrierDTOList: List.from(carrierDTOList)..removeLast(),
          );
          isNextPageAvailable = true;
        }

        return RemoteResponse<CarrierOutputDTO>(
          carrierOutputDTO,

...
import '../dto/address_output_dto.dart';
import 'address_enums.dart';
...
class AddressDataSource {
  ...
  AddressDataSource(this._dio, this._baseConfig);

  /// Retrieves addresses from the server based on specified filters, display
  /// options, and sort criteria.
  ///
  /// [filter] is an optional filter criteria for addresses.
  /// [display] is an optional display fields for addresses.
  /// [sort] is an optional sort criteria for addresses.
  ///
  /// Returns a `Future` that resolves to a `RemoteResponse<AddressOutputDTO>`
  /// object representing the response from the Prestashop API.
  Future<RemoteResponse<AddressOutputDTO>> getAddresses({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // If there are no addresses, return an empty list in the response
        if (response.data.toString() == '[]') {
          return const RemoteResponse<AddressOutputDTO>(
            AddressOutputDTO(addressDTOList: []),
          );
        }
        ...
        // Deserialize the JSON payload into a AddressOutputDTO instance
        final addressOutputDTO = AddressOutputDTO.fromJson(jsonMap);

        return RemoteResponse<AddressOutputDTO>(addressOutputDTO);
      } else {
  ...
  /// Retrieves a page of addresses from the server based on the specified page
  /// number, items per page, filters, display options, and sort criteria.
  ///
  /// [page] is the page number of addresses to retrieve.
  /// [perPage] is the maximum number of addresses to retrieve per page.
  /// [filter] is an optional filter criteria for addresses.
  /// [display] is an optional set of display fields to include in the response.
  /// [sort] is an optional sort order for addresses.
  ///
  /// Returns a `Future` that resolves to a `RemoteResponse<AddressOutputDTO>`
  /// object representing the response from the Prestashop API.
  Future<RemoteResponse<AddressOutputDTO>> getAddressesPage({
    required int languageId,
    required int page,
    ...
    final uriBuilder =
        UriBuilder(_baseConfig, addressesPath, languageId: languageId)
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Deserialize the JSON payload into a AddressOutputDTO instance
        AddressOutputDTO addressOutputDTO = AddressOutputDTO.fromJson(jsonMap);

        final addressDTOList = addressOutputDTO.addressDTOList;
        ...
        if (addressDTOList.length > perPage) {
          addressOutputDTO = addressOutputDTO.copyWith(
            addressDTOList: List.from(addressDTOList)..removeLast(),
          );
          isNextPageAvailable = true;
        }

        return RemoteResponse<AddressOutputDTO>(
          addressOutputDTO,
